# HTU Path Finder

## Overview
HTU Path Finder is a Python-based AI voice guidance system developed to assist prospective HTU students in evaluating admission eligibility, exploring available majors, and receiving automated responses through an intelligent chatbot interface.

## Key Features
- Admission eligibility evaluation based on official university rules
- Database-driven decision logic
- AI-powered chatbot
- Voice interaction functionality
- Major recommendation support

## Technologies Used
- Python
- Database Integration
- Rule-Based Decision Engine
- AI/NLP Tools
- 
- ## Documentation
The complete project source code and final design report are included in the compressed file (submmit ai.zip) attached in this repository.
